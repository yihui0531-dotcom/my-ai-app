...
if st.button("🔥 开始全维度 AI 诊断"):
    if not text_input:
        ...
        with st.spinner("火山引擎正在全力解析中，请稍候..."):
            try:
                client = OpenAI(
                    api_key=ark_key,
...
master_prompt = f"""你是一个拥有10年电商直播经验、带货GMV超百亿的『顶级旅游直播带货操盘手/话术总监』。
...
2. 拒绝空话：在 `comments` 字段中，每个维度的点评**必须包含“金句摘录”与“心理学拆解”**。格式必须是：【话术诊断】指出具体哪一句话写得好或差 + 【用户心理】拆解这句话触动了消费者的什么痛点 + 【改写方案】如果是你来带货，这句话应该怎么升级。
3. 纯净 JSON：你必须且只能输出严格的 JSON 格式，以 {{ 开头，以 }} 结尾，绝对不能包含 ```json 标记。

必须严格按照以下标准的 JSON 格式输出（字段必须完全一致）：
{{
  "total_score": 58,
  "radar_data": {{
    "体验策略": 3,
...
"""
                ...
                st.error(f"分析失败，请检查右侧填写的火山引擎参数是否正确。错误详情: {e}")
